...
def get_default_model():
    """获取默认模型，可通过环境变量DEEPSEEK_MODEL设置"""
...
class Agent:
    def __init__(self, instruction, role, examplers=None, model_info=None, img_path=None):
        ...
        self._model_name = self.model_info
        self.messages = [
    ...
    def temp_responses(self, message, img_path=None):
        ...
        for temperature in temperatures:
            ...
            token_usage_service.record_usage(
                provider="deepseek",
                model_name=self._model_name,
                source_type="mdagents_temp_responses",
                usage=response.usage.model_dump() if getattr(response, "usage", None) else None,
                success=True,
            )
...
class Group:
    def __init__(self, goal, members, question, examplers=None, model=None):
        self.goal = goal
        self.members = []
        default_model = model or get_default_model()
        for member_info in members:
            _agent = Agent('You are a {} who {}.'.format(member_info['role'], member_info['expertise_description'].lower()), role=member_info['role'], model_info=default_model)
...
def determine_difficulty(question, difficulty):
    ...
    medical_agent = Agent(instruction='You are a medical expert who conducts initial assessment and your job is to decide the difficulty/complexity of the medical query.', role='medical expert', model_info=default_model)
...
def process_intermediate_query(question, examplers, model, args):
    ...
    default_model = get_default_model()
    tmp_agent = Agent(instruction=recruit_prompt, role='recruiter', model_info=default_model)
